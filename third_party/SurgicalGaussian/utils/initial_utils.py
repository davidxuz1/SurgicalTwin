...
def get_all_initial_data_endo(path, data_type, depth_scale, is_mask, npy_file):
    ...
    if is_mask:
        ...
        inpaint_mask_all = np.zeros((H, W)) #edit
        ...
        for i in range(0, len(masks_path)):
            ...
            #m = cv.resize(m, (640, 512), interpolation=cv.INTER_AREA) #resize (edit)
    ...
    for i in range(poses_bounds.shape[0]):
        ...
        if is_mask:
            ...
            # Cuando no hay máscaras, usar una máscara de unos
            mask_image = np.ones((1, H, W))
        ...
        #inv_mask_all = cv.resize(inv_mask_all, (854, 480), interpolation=cv.INTER_NEAREST)  #resize (edit)
...
def sample_pts(height, width, factor=2):
    mask_sample_h = np.zeros((height, width)).astype(np.int32)
    mask_sample_w = np.zeros((height, width)).astype(np.int32)
