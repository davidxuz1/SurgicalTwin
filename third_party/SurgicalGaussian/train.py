...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        img_tv_loss = 0  # Valor por defecto (edit)
        if dataset.is_mask:
        ...
        if dataset.is_depth:
                ...
                if depth is not None:  # Añade esta verificación
                    depth = depth * mask_depth
                    depth_loss = l1_loss(depth, gt_depth)
                    loss = loss + 0.001 * depth_loss
                else:
                    print("Warning: Depth information not available from renderer")
                    depth_loss = None
        ...
        with torch.no_grad():
            ...
            # Antes de actualizar max_radii2D, asegúrate que las dimensiones coincidan
            if visibility_filter.dim() > 1:
                visibility_filter = visibility_filter.flatten()
